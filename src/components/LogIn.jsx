...
export const LogIn = () => {
    const { setUserIsLoggedIn, email, setEmail } = useContext(ShoppingCartContext)
    const [password, setPassword] = useState("")
    const navigate = useNavigate()

    // Este useEffect se ejecuta al cargar la página para verificar si venimos regresando de Google
    useEffect(() => {
        const checkRedirectResult = async () => {
            try {
                ...
                if (result && result.user) {
                    ...
                    alert("Log in con Google exitoso")
                    navigate("/") // Redirige al home
                }
            } catch (error) {
                console.error("Error al procesar el regreso de Google:", error)
            }
        }
        checkRedirectResult()
    }, [navigate, setUserIsLoggedIn])
    ...
    const SignInWithGoogle = async () => {
        try {
            // Ya no usamos Popup. Esto redirigirá la pestaña actual.
